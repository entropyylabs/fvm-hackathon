...
contract MockMarket {
    ...
    function publish_deal(bytes calldata raw_auth_params, uint256 proposalID)
        public
    {
...
contract DataDAO {
     ...
     */
    function createCIDProposal(bytes calldata cidraw, uint256 size) public {
        proposalCount++;
    ...
    function downvoteCIDProposal(uint256 proposalID) public {
        ...
        require(!hasVotedForProposal[msg.sender][proposalID], "Already Voted");
        require(isVotingOn(proposalID), "Voting Period Finished");
        proposals[proposalID].downVoteCount =
            proposals[proposalID].downVoteCount +
            1;
        hasVotedForProposal[msg.sender][proposalID] = true;
    }
    ...
    function policyOK(uint256 proposalID) public view returns (bool) {
        require(
            proposals[proposalID].proposalExpireAt > block.timestamp,
            "Voting in On"
        );
        return
            proposals[proposalID].upVoteCount >
    ...
    function authorizeData(
        ...
        require(
            policyOK(proposalID),
            "Deal failed policy check: Was the CID proposal Passed?"
        );
